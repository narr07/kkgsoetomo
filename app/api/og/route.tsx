...
export async function GET(request: Request) {
  try {
    ...
    // Truncate text (matching Nuxt implementation)
    const truncatedTitle = title.slice(0, 60);
    const truncatedDescription = description.slice(0, 200);
    const headline = 'KKG dr. SOETOMO';
    ...
    return new ImageResponse(
      (
        ...
        >
          {/* Background SVG - Blurred blob top right */}
          <svg
            style={{
              position: 'absolute',
              right: '0',
      ...
      ),
      {
        ...
        fonts: [
          {
            name: 'Inter',
            data: await loadGoogleFont('Inter', `${headline} ${truncatedTitle} ${truncatedDescription}`),
            style: 'normal',
          },
        ],
      },
